...
class gridScreens():
    def __init__(self,screenName = "Placing Ships"):
        self.grid = drawGrid()
    ...
    def checkIfGrid(self,position, locationGrid):
        gridKeys = []
        row = -1
        column = -1
    ...
    def getRectCoord(self, cord, locationGrid):
        tempKey = float(-1)
        tempCol = float(-1)
    ...
    def mouseHighlight(self,position, locationGrid, needShips):
    ...
    def checkSpot(self,shipLoc, ship, recLoc, color):
    ...
    def highlightedBoxes(self,ship, recLoc, locationGrid, color):
    ...
    def shipHighlight(self,position, locationGrid, ship, shipLoc):
        recLoc = []
        color = white
        drawGrid()
    ...
    def placingShips(self,position):
    ...
    def placingShipsRelease(self,position, locationGrid, sprite, onGrid):
    ...
    def shipIsHeld(self,position, sprite):
        drawGrid()
        sprite.set_location(position)
        ship_group_layered.draw(screen)
        pygame.display.update()

    def moveShipScreen(self,placing, run, grid, curSprite, shipLoc):
    ...
    def takeShotScreen(self,run, grid, clicked):
        is_hit = True
        pos = pygame.mouse.get_pos()
